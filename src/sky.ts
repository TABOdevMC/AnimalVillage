...
export function installSky(scene: THREE.Scene, camera: THREE.Camera) {
  ...
  // Nuages volumétriques : géométrie et matériau partagés pour limiter le coût GPU.
  ...
  // Soleil 3D : il n'apparaît que lorsque le ciel est réellement dégagé.
  ...
  // Précipitations 3D : pluie et neige dans le monde, jamais dans l'interface.
  const precipitation=new THREE.Group();
  precipitation.name='3DPrecipitation';
  const rainCount=520;
  const rainPositions=new Float32Array(rainCount*3);
  const rainVelocities=new Float32Array(rainCount*3);
  for(let i=0;i<rainCount;i++){
    const k=i*3;
    rainPositions[k]=(Math.random()-.5)*90;
    rainPositions[k+1]=4+Math.random()*42;
    rainPositions[k+2]=(Math.random()-.5)*90;
    rainVelocities[k]=-.6-Math.random()*.4;
    rainVelocities[k+1]=-18-Math.random()*18;
    rainVelocities[k+2]=.4+Math.random()*.7;
  }
  const rainGeo=new THREE.BufferGeometry();
  rainGeo.setAttribute('position',new THREE.BufferAttribute(rainPositions,3));
  const rainMat=new THREE.PointsMaterial({color:0xb8d9ef,size:.12,transparent:true,opacity:.72,depthWrite:false,sizeAttenuation:true});
  const rain=new THREE.Points(rainGeo,rainMat);
  rain.name='RainParticles';
  precipitation.add(rain);

  const snowPositions=new Float32Array(260*3);
  for(let i=0;i<260;i++){
    const k=i*3;
    snowPositions[k]=(Math.random()-.5)*90;
    snowPositions[k+1]=4+Math.random()*40;
    snowPositions[k+2]=(Math.random()-.5)*90;
  }
  const snowGeo=new THREE.BufferGeometry();
  snowGeo.setAttribute('position',new THREE.BufferAttribute(snowPositions,3));
  const snowMat=new THREE.PointsMaterial({color:0xffffff,size:.24,transparent:true,opacity:.9,depthWrite:false,sizeAttenuation:true});
  const snow=new THREE.Points(snowGeo,snowMat);
  snow.name='SnowParticles';
  precipitation.add(snow);
  precipitation.visible=false;
  sky.add(precipitation);

  // Éclairs 3D + flash lumineux projeté dans la scène.
  ...
  lightningGroup.add(makeBolt(glowMat,.13),makeBolt(boltMat,.055));
  ...
  scene.add(sky);
  const update=()=>{
    // Le ciel suit la caméra uniquement pour conserver les effets à bonne distance,
    // mais les nuages se déplacent en douceur : aucun téléport visuel à chaque déplacement.
    ...
    clouds.forEach((cloud,i)=>{
      cloud.visible=i<cloudCount;
      // Dérive très lente et continue des nuages dans le ciel.
      cloud.position.x=cloudPositions[i][0]+Math.sin(cloudTime*.012+i)*1.8;
      cloud.position.z=cloudPositions[i][2]+Math.cos(cloudTime*.01+i*.7)*1.2;
    });

    const clear=weather.includes('Beau temps')||weather.includes('Canicule');
    sunGroup.visible=clear;

    const rainy=weather.includes('Pluie')||weather.includes('Torrentielle')||weather.includes('Orage')||weather.includes('Ouragan');
    const snowy=weather.includes('Neige')||weather.includes('Tempête de neige');
    precipitation.visible=rainy||snowy;
    rain.visible=rainy;
    snow.visible=snowy;
    ...
    if(rainy){
      ...
      for(let i=0;i<rainCount;i++){
        const k=i*3;
        pos[k]+=rainVelocities[k]*dt;
        pos[k+1]+=rainVelocities[k+1]*dt;
        pos[k+2]+=rainVelocities[k+2]*dt;
        if(pos[k+1]<1){pos[k]=(Math.random()-.5)*90;pos[k+1]=38+Math.random()*10;pos[k+2]=(Math.random()-.5)*90;}
      }
    ...
    if(snowy){
      ...
      for(let i=0;i<260;i++){
        const k=i*3;
        pos[k]+=Math.sin(cloudTime+i)*.018;
        pos[k+1]-=dt*(1.1+((i%7)*.08));
        pos[k+2]+=Math.cos(cloudTime*.7+i)*.012;
        if(pos[k+1]<1){pos[k]=(Math.random()-.5)*90;pos[k+1]=38+Math.random()*7;pos[k+2]=(Math.random()-.5)*90;}
      }
