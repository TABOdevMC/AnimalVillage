...
export function installChunkedWorld(scene:THREE.Scene,camera:THREE.Camera):()=>void{
  ...
  const clearAroundBuildings=()=>{
    const buildings:any[]=[];
    scene.traverse((obj:any)=>{if(obj.userData?.building)buildings.push(obj)});
    if(!buildings.length)return;
    const targets:any[]=[];
    root.traverse((obj:any)=>{if(obj.isMesh||obj.isGroup)targets.push(obj)});
    for(const building of buildings){
      const b=building.userData.building;
      const size=Number(b?.size||3)*1.5;
      const center=new THREE.Vector3();building.getWorldPosition(center);
      for(const target of targets){
        if(!target.parent||target===root||target===building)continue;
